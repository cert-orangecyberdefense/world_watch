...
| ---------------------------------------- | ----------------------------------------- |
| `/rss/advisories`                        | Free advisories feed. No key required     |
| `/rss/updates`                           | Free updates feed. No key required        |
| `/rss/advisories/{RSS_API_KEY}?<filters>`| Premium advisories with optional filters. |
| `/rss/updates/{RSS_API_KEY}?<filters>`   | Premium updates with optional filters.    |
...
Each **Update**(`Content Block`) contains the actual data, the **Advisory** acts as an umbrella and
brings these blocks together under a common theme or context.
...
**N.B**: If you don’t have a manager account, [**email us**](mailto:worldwatch-request.ocd@orange.com) to request access.
